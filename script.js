...
async function agregarDato() {
  ...
  try {
    ...
    ['nombrePieza', 'codigoPieza', 'numFactura', 'proveedorPieza', 'valor', 'descripcion'].forEach(id => {
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .encabezado { display:flex; justify-content:space-between; align-items:flex-start; margin-top: 4px; }
  .encabezado .titulo { font-weight:700; font-size:13px; letter-spacing:.2px; }
  .encabezado .datos { text-align:right; font-size:12.5px; }
  .encabezado .datos .op-num { font-weight:700; }

  .beneficiario { margin-top: 26px; }
  .beneficiario .etiqueta { margin-bottom: 4px; }
  .beneficiario .nombre,
  .beneficiario .linea { font-style: italic; font-weight: 700; }
  ...
  thead th {
    text-align:left; font-weight:400; text-decoration:underline;
    padding-bottom:6px; font-size:12.5px;
  }
  thead th.num { text-align:right; }
  tbody td { padding: 3px 0; font-size:12.5px; vertical-align:top; }
  tbody td.num { text-align:right; white-space:nowrap; }
  tbody tr.espaciador td { padding-top: 8px; }

  .linea-total { border-top: 1px solid #444; margin-top: 8px; }
  .totales { margin-top: 6px; display:flex; justify-content:flex-end; }
  .totales table { width: auto; margin-top:0; }
  .totales td { padding: 2px 0 2px 28px; font-size:12.5px; }
  .totales td.label { font-weight:700; text-align:left; }
  .totales td.val { font-weight:700; text-align:right; }
  .totales tr.total-final td { font-size:13.5px; }

  .en-letras { margin-top: 14px; font-style: italic; }
  .obs { margin-top: 18px; }
  .obs .etiqueta { font-weight:700; }

  .despedida { margin-top: 26px; }

  .firmas { display:flex; justify-content:space-between; margin-top: 70px; gap: 24px; }
  .firma { flex:1; text-align:center; font-size: 11.5px; }
  .firma-linea { border-top:1px solid #444; margin-bottom:5px; padding-top:5px; font-weight:700; }

  .pie-footer {
    margin-top: 40px; text-align:center; font-size:10.5px; color:#c8860a; font-weight:600;
  }
  .pie-sistema {
    margin-top: 4px; text-align:center; font-size:9px; color:#999;
  }
</style>
</head><body>
  ...
  <div class="beneficiario">
    <div class="etiqueta">Beneficiario:</div>
    <div class="nombre">${(r.Proveedor || '—').toUpperCase()}</div>
    ${r.Factura ? `<div class="linea">Factura N° ${r.Factura}</div>` : ''}
  </div>

  <div class="parrafo">
    <div class="intro">De mi mayor consideración:</div>
    <div class="detalle">Se aprueba el pago correspondiente al presupuesto habilitado, según el siguiente detalle:</div>
  </div>
  ...
  <table>
    ...
    <tbody>
      <tr>
        <td>${r.Codigo || '—'}</td>
        <td>${r.Pieza}${r.Proyecto ? ` (${r.Proyecto})` : ''}</td>
        <td class="num">${r.Cantidad}</td>
        <td class="num">${formatearMonto(moneda, r.Precio_Unit || 0)}</td>
        <td class="num">${totalTexto}</td>
      </tr>
    </tbody>
  </table>
  ...
  <div class="totales">
    <table>
      <tr class="total-final"><td class="label">TOTAL</td><td class="val">${totalTexto}</td></tr>
    </table>
  </div>

  <div class="en-letras">${moneda === 'USD' ? 'Son dólares' : 'Son pesos'}: ${letras}.</div>

  ${r.Descripcion ? `<div class="obs"><span class="etiqueta">Observaciones:</span> ${r.Descripcion}</div>` : ''}

  <div class="despedida">Sin otro particular, saludo Atte.,</div>

  <div class="firmas">
    <div class="firma"><div class="firma-linea">Confeccionó</div>${confeccionoNombre}</div>
    <div class="firma"><div class="firma-linea">Aprobó</div>&nbsp;</div>
    <div class="firma"><div class="firma-linea">Recibí</div>&nbsp;</div>
  </div>

  <div class="pie-footer">Av. Las Heras 3156 – Brandsen – Pcia. de Buenos Aires - Argentina</div>
  <div class="pie-sistema">Documento generado desde Gestión de Pagos MTTO — ${fechaHoraCompleta} · ${confeccionoNombre}</div>
